...
def configurar_pagina():
    ...
    st.set_page_config(page_title="Astra IA", page_icon= "🤖", layout="centered")

    st.title("Astra IA")

    st.sidebar.title("Elección de modelo")
...
def generar_respuesta(respuesta_completa_del_bot):
    ...
    for frase in respuesta_completa_del_bot:
        if frase.choices[0].delta.content:
            _respuesta_enserio += frase.choices[0].delta.content
            yield frase.choices[0].delta.content
    return _respuesta_enserio
...
def main():
    ...
    promt_del_usuario = st.chat_input("¿En qué te puedo ayudar hoy?")
    ...
    if promt_del_usuario:
        ...
        if respuesta_del_bot:
            with st.chat_message("assistant"):
                respuesta_enserio = st.write_stream(generar_respuesta(respuesta_del_bot))
                actualizar_historial("assistant", respuesta_enserio, "🤖")
